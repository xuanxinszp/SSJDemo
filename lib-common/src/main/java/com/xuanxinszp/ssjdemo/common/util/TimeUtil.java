...
public class TimeUtil {
    ...
    public static Date currentDateTime(){
        Date date = now();
        try {
            String dateStr = DATETIME_FORMAT.format(date);
    ...
    public static boolean nowIsBetween(int startDay, int endDay) {
       String nowDay = toString(now(), "dd");
        if(nowDay.startsWith("0")) {
            nowDay = nowDay.substring(1,2);
        }
        if(startDay <= Integer.valueOf(nowDay) && Integer.valueOf(nowDay) <= endDay) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public static String getFrontMonth(int front) {
        Calendar startCale = Calendar.getInstance();//获取当前日期
        startCale.add(Calendar.MONTH, -front);
        return new DateTime(startCale).toString(monthPattern);
    }
     ...
     * @throws Exception
     */
    public static String getFrontMonth(String month, int front) throws Exception{
        SimpleDateFormat sdf = new SimpleDateFormat(monthPattern);
        Calendar startCale = Calendar.getInstance();//获取当前日期
        startCale.setTime(sdf.parse(month));
        startCale.add(Calendar.MONTH, -front);
        return new DateTime(startCale).toString(monthPattern);
    }
     ...
     * @throws Exception
     */
    public static String getLastMonthByMonth(String month, int front) throws Exception{
        if(front <= 0) {
            return month;
        }
        SimpleDateFormat sdf = new SimpleDateFormat(monthPattern);
    ...
    /**
     * 根据日期获取后多个月，格式YYYY-MM
     * @param date YYYY-MM-DD
     * @param front
     * @return
     */
    public static String getLastMonth(String date, int front) {
        Calendar startCale = Calendar.getInstance();//获取当前日期
        startCale.setTime(stringToDate(date, datePattern));
        startCale.add(Calendar.MONTH, front);
        return new DateTime(startCale).toString(monthPattern);
    }
    ...
    /**
     * 两个日期相差的月份数
     * @param smonth yyyy-mm
     * @param bmonth yyyy-mm
     * @return
     */
    public static int monthsBetween(String smonth, String bmonth) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat(monthPattern);
        Calendar bef = Calendar.getInstance();
        Calendar aft = Calendar.getInstance();
        bef.setTime(sdf.parse(smonth));
        aft.setTime(sdf.parse(bmonth));
     ...
     * @return
     */
    public static String getLatelyYearDate(String date) {
        int daysBetween = daysBetween(date, dateFormat(now()));
        int year =(int) Math.floor(daysBetween / 365);
        Calendar startCale = Calendar.getInstance();
        startCale.setTime(stringToDate(date, datePattern));
        startCale.add(Calendar.YEAR, year);
        return new DateTime(startCale).toString(datePattern);
    }
    ...
    public static List<String> dayReport(Date month) {
        ...
        List<String> monthDays = Lists.newArrayList();
        for (int i = 0; i < dayNumOfMonth; i++, cal.add(Calendar.DATE, 1)) {
            Date d = cal.getTime();
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat(datePattern);
            monthDays.add(simpleDateFormat.format(d));
        }
        return monthDays;
    }
...
//        dayComparePrecise( TimeUtil.stringToDate("2017-11-08"),TimeUtil.now());

        System.out.println("2018-09".substring(5));
        System.out.println(getLastMonthByMonth("2018-09", 13));
        System.out.println(monthsBetween("2018-09","2019-10"));
//        long min = getDistanceTime(TimeUtil.stringToDate(TimeUtil.getDate(TimeUtil.now())), TimeUtil.stringToDate("2018-09-08"));
//
//        int day = (int) (min /(24*60)) ;
